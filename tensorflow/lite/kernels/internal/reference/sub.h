...
void BroadcastSubSlow(const ArithmeticParams& params,
  ...
  BroadcastSubCommon<T>(
      ...
      [](T input1_val, T input2_val, const ArithmeticParams& lambda_params) {
        T activation_min, activation_max;
        GetActivationParams(lambda_params, &activation_min, &activation_max);
...
inline void BroadcastSub16POTSlow(const ArithmeticParams& params,
  ...
  BroadcastSubCommon<int16_t>(
      ...
      [](int16_t input1_val, int16_t input2_val,
         const ArithmeticParams& lambda_params) {
        const int32_t scaled_input1_val =
            gemmlowp::RoundingDivideByPOT(input1_val, -lambda_params.input1_shift);
        const int32_t scaled_input2_val =
            gemmlowp::RoundingDivideByPOT(input2_val, -lambda_params.input2_shift);
        const int32_t raw_output = scaled_input1_val - scaled_input2_val;
        const int32_t clamped_output =
            std::min(lambda_params.quantized_activation_max,
                     std::max(lambda_params.quantized_activation_min, raw_output));
  ...
  BroadcastSubCommon<T>(
      ...
      [](T input1_val, T input2_val, const ArithmeticParams& lambda_params) {
        const int32_t shifted_input1_val =
            (lambda_params.input1_offset + input1_val) * (1 << lambda_params.left_shift);
        const int32_t shifted_input2_val =
            (lambda_params.input2_offset + input2_val) * (1 << lambda_params.left_shift);
        const int32_t scaled_input1_val =
            MultiplyByQuantizedMultiplierSmallerThanOneExp(
                shifted_input1_val, lambda_params.input1_multiplier,
                lambda_params.input1_shift);
        const int32_t scaled_input2_val =
            MultiplyByQuantizedMultiplierSmallerThanOneExp(
                shifted_input2_val, lambda_params.input2_multiplier,
                lambda_params.input2_shift);
        const int32_t raw_sub = scaled_input1_val - scaled_input2_val;
        const int32_t raw_output =
            MultiplyByQuantizedMultiplierSmallerThanOneExp(
                raw_sub, lambda_params.output_multiplier, lambda_params.output_shift) +
            lambda_params.output_offset;
        const int32_t clamped_output =
            std::min(lambda_params.quantized_activation_max,
                     std::max(lambda_params.quantized_activation_min, raw_output));
