...
TfLiteStatus CalculateOpData(TfLiteContext* context, TfLiteAddParams* params,
  ...
  if (output->type == kTfLiteInt8) {
    ...
    data->params.output_multiplier = 1.0f / output->params.scale;
...
TfLiteStatus EvalAddQuantized(TfLiteContext* context, TfLiteNode* node,
  ...
  if (need_broadcast) {
    ...
    sli_mvp_ml_add_s8_params_t add_s8_params = data->params;
    add_s8_params.input1 = tflite::micro::GetTensorData<int8_t>(input1);
    add_s8_params.input2 = tflite::micro::GetTensorData<int8_t>(input2);
    add_s8_params.output = tflite::micro::GetTensorData<int8_t>(output);
    sl_status_t ret = sli_mvp_ml_add_s8(&add_s8_params);
    if (ret != SL_STATUS_OK) {
        status = kTfLiteError;
    }
  }
